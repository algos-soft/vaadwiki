...
public class QueryLoginTest extends ATest {
    ...
    @Test
    @Order(1)
    @DisplayName("urlRequest di queryLogin")
    void urlRequest() {
        previsto = JSON_SUCCESS;
        ottenuto = istanza.urlRequest();
        ottenutoBooleano = istanza.isLoginValido();
        assertTrue(ottenutoBooleano);
        assertEquals(ottenuto, previsto);
        assertEquals(botLogin.getLguserid(), QueryLogin.LG_ID);
        assertEquals(botLogin.getLgusername(), QueryLogin.LG_NAME);
        System.out.println(VUOTA);
        System.out.println(String.format("Risultato della request: %s", ottenuto));
        ...
        System.out.println(String.format("Cookies: memorizzati in BotLogin"));
    }

    @Test
    @Order(2)
    @DisplayName("urlRequest di queryAssert")
    void urlRequest2() {
        Map mappa = botLogin.getCookies();

        ottenutoBooleano = queryAssert.urlRequest(mappa);
        assertTrue(ottenutoBooleano);
        assertEquals(botLogin.getLguserid(), QueryLogin.LG_ID);
        assertEquals(botLogin.getLgusername(), QueryLogin.LG_NAME);
        System.out.println(VUOTA);
